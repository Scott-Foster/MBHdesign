...
\footnotetext{This is an introduction to the MBHdesign R-pacakge (available from \url{https://cran.r-project.org/package=MBHdesign}). Its development started as part of a NESP Marine Biodiversity Hub Project \url{https://www.nespmarine.edu.au/project/project-d2-standard-operating-procedures-survey-design-condition-assessment-and-trend} but extra functionality is added when need arises.}
...
@
\noindent You will be asked which repository you want to use. Just use one that is geographically close to where you are (or where your computer is). The second way to install \texttt{MBHdesign} is to via the github repository, which is updated more frequently. Once again, start R and submit
<<setup1a, eval=FALSE>>=
devtools::install_github( repo="Scott-Foster/MBHdesign")
@
...
workProbs <- alterInclProbs.cluster( clusterSize=5,
...
Since then, its life has been moulded by the needs of other projects -- notably the National Koala Monitoring Program \url{https://www.csiro.au/en/research/indigenous-science/managing-country/koala-monitoring-program}.
